<?php 
//this php file lets you create an agent
//powered by zcino
...
if (isset($_POST['add'])) { 
			...
			if ($ok!==false) {
				...
				$time = date('H:i:s');
				mysqli_query($GLOBALS['con'],"INSERT INTO `cws_staffs` (`login`,`pass`,`name`,`email`,`cash`,`status`,`owner`,`staff_type`,`percent`,`has_subagent`) VALUES ('$login','".pass_encode($pass)."','$name','$email','$cash','$status','{$_SESSION['admin']}','agent','$percent','$has_subagent')") or error_report(mysqli_error($GLOBALS['con']));
				...
				if ($cash>0){
					mysqli_query($GLOBALS['con'],"INSERT INTO `cws_transfers` (sender_id,sender_cash_b4,receiver_id,amount,date,sender_type,receiver_type,status) VALUES ('$staffid','$agentCash','$receiverid','$cash','$date','admin','admin','1')") or error_report(mysqli_error($GLOBALS['con']));
				}				
				...
				echo 'Agent '.$login.' '.$lang['created+successfully'];
				echo '</p></div>';
			} else {
				echo '<div class="nNote nFailure hideit">
...
<fieldset>
<div class="widget">
<div class="title"><img class="titleIcon" alt="" src="images/icons/dark/list.png" /><h6><?=$lang['Create+Agent']?></h6></div>
...
<div class="formRight"><input type="text" class="text small" name="smallfield" id="pass" value="<?php echo antisqli(urldecode($_POST['pass']));?>"  style="width:170px"/></div>
<div class="clear"></div>
</div>
...
<div class="formRight"><input type="text" class="text small" name="smallfield" id="cash" value="<?=isset($_POST['cash'])?antisqli($_POST['cash']):'0.00'?>" style="width:170px;color:green;"/> <?=$_SESSION['currency']?></div>
<div class="clear"></div>
</div>
...
<br />
<span style="font-size:9px"> <b><?php
 	if ($_SESSION['adminlvl']!=='admin'){
...
<div class="formRight"><select id="has_subagent">
...
<a href="#updated" class="button dblueB" id="add" style="padding:5px 49px 10px 40px"><span><?=$lang['Create+Agent']?></span></a><br />
...
$("#add").click(function() {
				...
				showparam('um_create_a','add=1&'+'login='+login+'&'+'has_subagent='+has_subagent+'&'+'percent='+percent+'&'+'pass='+pass+'&'+'email='+email+'&'+'name='+name+'&'+'fam='+fam+'&'+'status='+status+'&'+'cash='+cash);
							 });
</script>
</div>
</fieldset>
