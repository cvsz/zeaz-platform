<?php
//this php file lists all the operators of the current logged in staff
//powered by zcino
...
function delete_person(id){
	...
	if (answer){
		showparam('um_list_o','delete=1&id='+id);
	}
}
	$(function() {
...
$("#search").click(function() {
				...
				showparam('um_list_<?=$ext?>','search='+lookfor+'&searchemail='+searchemail+'&fromdate='+fromdate+'&todate='+todate<?php if (isset($_POST['owner'])){echo "+'&owner=".$_POST['owner']."'";}?>);
							 });
</script>
<?php 
...
<h3 style="margin-left:10px;"><?=ucfirst($stafftype).'s'?> <span style="color:red">
<?php
require_once('../config.inc.php');
...
<div class="widget">
    <div class="title">
    <img src="images/icons/dark/full2.png" alt="" class="titleIcon" /><h6>List Operators</h6>
    </div>  
...
<th width="8%" class="top acenter"><?=$lang['Total']?> Operator <?=$lang['Revenue']?></th>
<th width="8%" class="top acenter"><?=$lang['Total+Amount+Paid+to+Operator']?> </th>
...
<tr class="<?=($o%2)?'gradeA odd':'gradeA even'?>">
                   ...
                   <!-- purchased credits -->
                    ...
                    <div class="manageTd">
                    <?php if ($row['status']=='1') { 
						echo '<a onclick="javascript:showparam(\'um_list_'.$ext.'\',\'status=2&id='.$row['id'].'\');" href="#show" class="button manage blackB tipS" original-title="'.$lang['Disable'].'"><img class="icon2" alt="'.$lang['Disable'].' tipS" original-title="'.$lang['Disable'].'"src="images/icons/light/block.png" style="vertical-align:middle"></a>';;
						...
						?> 
                        <a onclick="javascript:showparam('um_edit_<?=$ext?>','edit=1&id=<?=$row['id']?>');" href="#show" class="button manage blueB tipS" original-title="<?=$lang['Edit']?>"><img class="icon2" alt="<?=$lang['Edit']?>" title="<?=$lang['Edit']?>"  src="images/icons/light/pencil.png" style="vertical-align:middle" /><span></span></a>
                        <?php if ($_SESSION['adminlvl']=='admin'){?><a onclick="javascript:delete_person('<?=$row['id']?>')"  href="#show" class="button manage redB tipS"  original-title="<?=$lang['Delete']?>"><img class="icon2" alt="<?=$lang['Delete']?>" title="<?=$lang['Delete']?>" src="images/icons/light/close.png" style="vertical-align:middle" /><span></span></a><?php } ?>
					</div>
                    </td>
               				  </tr>
<?php
...
<?php
require_once('_inc_pages.inc.php');
?>
</div>