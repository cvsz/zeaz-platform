<?php 
//this php file lets you edit the selected payment method
//powered by zcino
...
if (isset($_POST['update']) && $_SESSION['adminlvl']=='admin') {
			...
			if (!is_numeric($id) || $id<0){
				...
				<p><strong>'.$lang['Incorrect+value'].': '.$lang['Invalid+name'].'</strong></p></div>';
							...
							echo '<div class="nNote nFailure hideit">
				<p><strong>'.$lang['Incorrect+value'].': '.$lang['Phone+number+too+long'].'</strong></p></div>';
						}
						echo '<script type="text/javascript">$("#name").css("border","2px solid #F00");</script>';
					}else{mysqli_query($GLOBALS['con'],"UPDATE `cws_depositsettings` SET `name`='$name' WHERE `id`='$id'") or error_report(mysqli_error($GLOBALS['con']));}
				...
				<p><strong>'.$lang['Incorrect+value'].': '.$lang['Invalid+status'].'</strong></p></div>';
					...
					}else{mysqli_query($GLOBALS['con'],"UPDATE `cws_depositsettings` SET `status`='$status' WHERE `id`='$id'") or error_report(mysqli_error($GLOBALS['con']));}
				...
				<p><strong>'.$lang['Incorrect+value'].': '.$lang['Invalid+email'].'</strong></p></div>';
					...
					}else{mysqli_query($GLOBALS['con'],"UPDATE `cws_depositsettings` SET `deposit_email`='$email' WHERE `id`='$id'") or error_report(mysqli_error($GLOBALS['con']));}
...
$row = mysqli_fetch_array(mysqli_query($GLOBALS['con'],"SELECT * FROM  cws_depositsettings WHERE `id`='".antisqli($_POST['id'])."'"));
?>
<form name="form" onsubmit="return false" class="form" style="text-align:left">
<fieldset>
 <div class="widget">
	<div class="title"><img class="titleIcon" alt="" src="images/icons/dark/list.png"><h6><?=$lang['Edit+Payment+Merchant']?> - <span style="font-style:italic;color:#060">"<?php if (isset($_POST['id'])) { echo mysqli_result(mysqli_query($GLOBALS['con'],"SELECT `name` FROM `cws_depositsettings` WHERE `id`='".antisqli($_POST['id'])."'"),0);}?>"</span></h6></div>
    <div class="formRow">
    ...
    <div class="formRow">
        ...
        <div class="formRight"><input type="text" class="text small" name="smallfield" id="email" value="<?=$row['deposit_email']?>" style="width:235px"/></div>
        <div class="clear"></div>
    </div>
<div class="formRow">
...
<a style="margin: 5px;" class="button dblueB" id="update" title="<?=$lang['Update']?>" href="#" onclick="updatec()"><span><?=$lang['Update']?></span></a>
<script type="text/javascript">
$("#update").click(function() {
				var name = $("#name").val();
				var email = $("#email").val();
				var status = $("#status option:selected").val();
				showparam('cas_payments_e','update=1&'+'name='+name+'&'+'email='+email+'&'+'status='+status+'&'+'id='+<?=antisqli($_POST['id'])?>);
							 });
</script>
</fieldset>
</form>
