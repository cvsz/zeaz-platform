...
<div style="padding-top:20px;text-align:center"><h2>Multiplayer EU Roulette - <?php echo $lang['Gameplay+history']?></h2></div>
...
$query = "SELECT id,(SELECT number FROM cws_roulette_eu_results WHERE id=x.drawid) AS number, drawid,'ROULETTE EU' as name,date,bet,sum_won FROM cws_roulette_eu_bets x WHERE user='{$_SESSION['username']}' AND mode='real'";
