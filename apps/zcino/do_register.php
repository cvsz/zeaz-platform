...
$formx = '<table>
					...
					<td><input class="field" type="text" name="usernameReg" id="usernameReg"  value="'.(isset($_POST['usernameReg']) ? htmlspecialchars($_POST['usernameReg']) : '').'"/></td></tr>
                    <tr>
					<td class="grey" style="width:140px;float:left">'.$lang['Email'].':</td>
					<td><input class="field" type="text" name="emailReg" id="emailReg"  value="'.(isset($_POST['emailReg']) ? htmlspecialchars($_POST['emailReg']) : '').'" /></td></tr>
                    <tr>
...
<div id="openAccount">
<?php
$qee = mysqli_query($GLOBALS['con'],"SELECT `ip_reg` FROM `cws_users_info` WHERE id='{$_SESSION['userid']}'") or die(mysqli_error($GLOBALS['con']));
echo '<span style="color:white;font-weight:bold;font-size:11px;">'.$lang['Registration+IP'].'</span>:<span style="color:orange">'.mysqli_result($qee,0).'</span><br /> ';
echo '<span style="color:white;font-weight:bold;font-size:11px;">'.$lang['Last+login+IP'].'</span>:<span style="color:orange">'.$_SESSION['lastIP'].'</span>';
?>
...
$l = $_SESSION['username'];
$i = 0;
$query = "SELECT cws_gameplays.id as id,cws_gameplays.date as date,bet,won,cws_games.name as Gname FROM cws_gameplays INNER JOIN `cws_games` ON cws_gameplays.gamename=cws_games.id WHERE cws_gameplays.user='$l' ORDER BY cws_gameplays.date DESC LIMIT 0,3";
