...
if (isset($_POST['pw'])) {if ($_SESSION['temp_password']==$_POST['pw']){echo '<span style="color:green">OK</span>';}else{echo $lang['Passwords+do+not+match'];}}
else {
if (isset($_POST['submit1'])) {
	if ($_SESSION['registered']!==TRUE) {
		if (isset($_POST['submit1'])) {
				 $user_ip_address = $_SERVER["REMOTE_ADDR"];
				 if (!is_numeric($_POST['mobiletel'])) {$errormsg = 'mobiletel';}
				 foreach($_POST as $key => $val) {
				  		 $_POST[$key] = antisqli(validateInput(trim($value))); // validate all data that can be sent by users input
						 $reserved_kw = array('GLOBALS','_SERVER','_GET','_POST','_FILES','_REQUEST','_SESSION','_ENV','_COOKIE','php_errormsg','HTTP_RAW_POST_DATA','http_response_header','argc','argv');
				 ...
				 if ($_SESSION['temp_password']!==$_POST['pass']) {$errormsg = 'pass';}
				 if (!check_pw($_POST['pass'])) {$errormsg = 'pass';}
				 ...
				 if(empty($errormsg)){ 
								if (mysqli_query($GLOBALS['con'],$sql)) {
									...
									$sql2 = "INSERT INTO `cws_users_info` (`id`,`name`,`email`,`pass`,`gender`,`dob`,`ip_reg`,`street`,`zip`,`ort`,`country`,`mobiletel`,`secques`,`secans`,`aff_id`) VALUES ('$userid','$name','".antisqli($email)."','".pass_encode($pass)."','".antisqli($_POST['gender'])."','".antisqli($_POST['dob'])."','".antisqli($user_ip_address)."','".antisqli($_POST['street'])."','".antisqli($_POST['zip'])."','".antisqli($_POST['ort'])."','".antisqli($_POST['country'])."','".antisqli($_POST['mobiletel'])."','".antisqli($_POST['secques'])."','".antisqli($_POST['secans'])."','".$aff_id."')";
								...
								} else {die($sql.'<br />'.mysqli_error($GLOBALS['con']));}
						 }
							
				 }
} else {$errormsg = 'registered';}
echo $errormsg;
}else {
...
<script type="text/javascript">
	...
	function check(parameter) {
		...
		if (parameter == 'pass') {
			$.post('do_activate.php',{pw:param},function(data) {$("#pass_error").html(data);
	...
	function doActivate() {
		...
		$.post('do_activate.php',{affiliate:affiliate,pass: pass,gender:gender,name:name,fam:fam,dob:dob,street:street,zip:zip,ort:ort,country:country,mobiletel:mobiletel,secques:secques,secans:secans,submit1:'submit1'},
			function(msg) {
...
<table width="590" border="0" cellpadding="0" cellspacing="0" class="openAccTable">
                        ...
                        <tr><td>Username: </td><td><strong><?=$_SESSION['temp_username'];?></strong></td>
                          <td>&nbsp;</td></tr>
                        <tr>
					  ...
					  <tr>
                        ...
                        <tr>
							<td class="bold">Affiliate: </td>
							<td><input name="affiliate" type="text" class="normal" id="affiliate" style="width:180px;" value="<?php  echo $_SESSION['aff']; ?>" onKeyUp="check('affiliate')"/></td>
						...
						<tr>
							<td colspan="3" height="30">
								...
								<a href="terms.php" target="_blank">terms and conditions</a> and confirm that I am of 18 years or over.							</td>
						</tr>
                        <tr>
                        <td></td>
                        <td></td>
