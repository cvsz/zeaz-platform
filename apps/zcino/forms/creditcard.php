...
<h3>Deposit {amount} {currency} via Credit/Debit Card</h3>
...
<br /><br />
<button id="confirm-card-deposit" style="background:#007BFF; color:#fff; padding:12px 24px; font-weight:bold; border:none; border-radius:5px; cursor:pointer; width:80%; font-size:16px;">Pay {amount} {currency}</button>
<br /><br />
<div id="card-status" style="color:yellow; font-weight:bold;"></div>
<br />
...
$("#confirm-card-deposit").click(function() {
    $("#card-status").css("color", "yellow").text("Processing payment via Secure Gateway...");
    ...
    setTimeout(function() {
        $.post('includes/do_stripe_deposit_backend.php', { amount: "{amount}" }, function(data) {
            if(data.trim() == "Success") {
                $("#card-status").css("color", "#00e701").text("Payment Successful! Crediting account...");
                setTimeout(function() {
                    $('#loginDiv').fadeOut('slow', function() {
                ...
                $("#card-status").css("color", "red").text("Error: " + data);
                $("#confirm-card-deposit").prop("disabled", false).css("opacity", "1");
            }
        });
    }, 2000);
});
