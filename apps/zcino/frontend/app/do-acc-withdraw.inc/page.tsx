...
export default function DoAccWithdrawIncPage() {
  return (
    <div className="mx-auto min-h-screen max-w-[90rem] px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-8 overflow-hidden rounded-[2rem] border border-border bg-card/80 p-6 shadow-card backdrop-blur-xl sm:p-8 lg:p-10">
        ...
        <p className="mt-4 max-w-2xl text-base leading-7 text-muted sm:text-lg">
          Process a withdrawal for a player.
        </p>
      </header>
      <div className="mx-auto max-w-2xl">
      <Card><CardHeader><CardTitle>Process Withdrawal</CardTitle><CardDescription>Admin withdrawal processing.</CardDescription></CardHeader><div className="space-y-4">
      ...
      <div><label className="mb-1 block text-sm font-bold text-foreground">Withdrawal Amount</label><input className="w-full rounded-xl border border-border bg-card px-4 py-2.5 text-foreground placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-cyan-400" type="number" placeholder="0.00" /></div>
      ...
      <div className="flex gap-3 pt-2"><Button>Process Withdrawal</Button><Button variant="secondary">Cancel</Button></div>
      </div></Card>
      </div>
    </div>
  );
