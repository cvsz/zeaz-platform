...
`apps/web` is the main ZeaZ public web frontend inside `cvsz/zeaz-platform`.
...
This README documents only `apps/web`. Do not mix commands from `apps/api`, `apps/zoffice`, `apps/zdash`, or other app stacks.

## Local development

```bash
cd /home/zeazdev/zeaz-platform/apps/web
npm install
npm run dev
```
