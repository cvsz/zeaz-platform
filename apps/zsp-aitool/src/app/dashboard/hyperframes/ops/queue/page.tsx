...
export default function HyperFramesOpsQueuePage() {
  ...
  const warnings = useMemo(() => !status ? ["ไม่มีคำสั่งควบคุมระบบใน UI"] : ["ไม่มีคำสั่งควบคุมระบบใน UI", status.staleRunning > 0 ? `พบ stale running ${status.staleRunning} งาน` : "ไม่พบ stale running", "การ recover/cleanup ต้องทำผ่าน operator script ที่มี confirmation gate"], [status]);
  const cards = !status ? [] : [
