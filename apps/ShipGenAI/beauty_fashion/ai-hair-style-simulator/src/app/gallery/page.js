...
import {
  FaDownload, FaSpinner, FaGoogle, FaImages, FaEye, FaCut, FaPlus, FaTrashAlt
} from "react-icons/fa";
...
export default function GalleryPage() {
  ...
  const handleDownload = async (creation) => {
    ...
    try {
      const response = await fetch(creation.resultImage);
      const blob = await response.blob();
      const blobUrl = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = blobUrl;
      a.download = `hairstyle-${creation.id}.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(blobUrl);
    } catch (e) {
      window.open(creation.resultImage, "_blank");
    }
  };

  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this hairstyle simulation? This action cannot be undone.")) return;
    setDeletingId(id);
    try {
      ...
      <div className="flex-1 flex items-center justify-center bg-zinc-955 px-4 py-12">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center shadow-xl">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed mb-8">
            Please sign in to access your HD gallery, view detail comparisons, and download simulated haircuts.
          </p>
          <button
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs sm:text-sm text-zinc-400 mt-1.5 font-medium font-sans">Browse your completed high-resolution AI hairstyles</p>
          </div>
          <Link
            href="/"
        ...
        {creations.length === 0 ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 text-center shadow-lg max-w-xl mx-auto my-12">
            ...
            <h2 className="text-lg font-bold text-zinc-200 mb-2">No completed hairstyles yet</h2>
            <p className="text-sm text-zinc-400 leading-relaxed max-w-sm mx-auto mb-8 font-medium">
              You don't have any successfully finished style designs in your showroom yet. Trigger a style makeover in the studio!
            </p>
            <Link
              href="/"
            ...
            >
              <FaPlus className="text-xs" /> Design New Style
            </Link>
          </div>
        ) : (
          /* Gallery Grid */
              ...
              >
                ...
                <div className="relative aspect-[4/5] bg-zinc-950 overflow-hidden flex items-center justify-center">
                  ...
                  <span className="absolute top-3 left-3 text-[8px] font-bold text-violet-300 bg-zinc-950 border border-zinc-800 px-2 py-0.5 rounded-lg shadow uppercase">
                    {creation.colorName} • {creation.styleName}
                  </span>
                </div>
        ...
        {selectedCreation && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md transition-opacity" onClick={() => setSelectedCreation(null)}>
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200 flex flex-col overflow-hidden max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                ...
                <div>
                  <h3 className="text-sm sm:text-base font-bold font-heading text-white flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold text-violet-400 bg-violet-950 border border-violet-850 px-2 py-0.5 rounded-lg uppercase">
                      {selectedCreation.colorName} • {selectedCreation.styleName}
                    </span>
                  </h3>
                </div>
                <button
              ...
              <div className="flex-1 flex items-center justify-center overflow-hidden min-h-0 relative select-none">
                <div className="relative w-full aspect-[4/5] rounded-xl overflow-hidden border border-zinc-800 bg-zinc-950 shadow-md select-none max-h-[60vh] max-w-sm">
                  ...
                  {/* Input reference overlay badge */}
                  <div className="absolute bottom-3 right-3 bg-zinc-900/95 border border-zinc-800 p-2.5 rounded-lg flex flex-col gap-1.5 z-20 shadow-lg max-w-[100px] backdrop-blur">
                    <span className="text-[7px] font-bold text-zinc-400 uppercase tracking-wider">Before</span>
                    <div className="h-10 w-8 rounded overflow-hidden border border-zinc-800 bg-zinc-950">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={selectedCreation.inputImage} alt="Selfie" className="w-full h-full object-cover" />
                    </div>
                  </div>

                  {/* Label badge */}
                  <div className="absolute top-3 left-3 bg-zinc-950/90 backdrop-blur border border-zinc-800 text-violet-400 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-lg z-20">
                    Simulated Look
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-zinc-800 pt-4 mt-4 flex justify-between items-center gap-3 flex-shrink-0">
                ...
                >
                  <FaTrashAlt className="text-[10px]" /> Delete Result
                </button>
