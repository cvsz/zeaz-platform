...
import { FaCoins, FaUser, FaSignOutAlt, FaGoogle, FaBars, FaTimes, FaPenNib } from "react-icons/fa";
import { SiVercel } from "react-icons/si";
import clsx from "clsx";

const navLinks = [
  { name: "Tattoo Studio", href: "/" },
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3.5 bg-zinc-950/80 border-b border-zinc-800/50 backdrop-blur-md text-zinc-100 flex-shrink-0">
      ...
      <div className="flex items-center gap-5 sm:gap-7 min-w-0">
        <Link href="/" className="flex items-center gap-2 font-extrabold tracking-tight text-white flex-shrink-0 group">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-violet-600 to-fuchsia-500 flex items-center justify-center text-white shadow-lg shadow-violet-500/20 group-hover:scale-105 transition-transform duration-200">
            <FaPenNib className="text-sm rotate-45" />
          </div>
          <span className="text-sm sm:text-base leading-none">
            Tattoo<span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400 font-black">AI</span>
          </span>
        </Link>
      ...
      <div className="hidden md:flex items-center gap-2 sm:gap-3 flex-shrink-0">
        {/* Deploy button */}
        <a
          href="https://vercel.com/new/clone?repository-url=https://github.com/SamurAIGPT/ai-tattoo-try-on"
          target="_blank"
        ...
        {session?.user ? (
          <>
            ...
            <button
              onClick={() => signOut()}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-zinc-500 hover:text-red-400 hover:bg-red-955/20 transition-all cursor-pointer"
              title="Sign out"
            >
      ...
      <div className="flex items-center gap-2 md:hidden">
        {session?.user && (
          <span className="flex items-center gap-1 text-[11px] font-bold text-amber-300 bg-amber-955/30 border border-amber-800/40 px-2.5 py-1 rounded-full">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 z-[200] flex flex-col p-4 bg-zinc-955 border-b border-zinc-800 backdrop-blur-lg md:hidden gap-3 shadow-xl">
              ...
              return (
                <Link
                  ...
                  className={clsx(
                    ...
                    isActive
                      ? "text-violet-400 bg-violet-955/20 border-l-2 border-violet-500"
