...
const REPO_URL = "https://github.com/jaiprasad04/ai-travel-studio";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-800 bg-zinc-950/95 backdrop-blur-md flex-shrink-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center h-14 gap-4">
        ...
        >
          <span className="h-7 w-7 rounded-lg flex items-center justify-center bg-gradient-to-br from-teal-400 to-emerald-500 shadow-lg shadow-teal-500/20">
            <FaGlobe className="text-white text-xs animate-spin-slow" />
          </span>
          <span className="text-sm font-semibold tracking-tight">
            <span className="text-white">FutureTravel</span>
        ...
        <div className="hidden md:flex items-center gap-3 ml-auto">
          ...
          {/* Credits */}
          {session?.user && (
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-teal-400 bg-teal-400/10 border border-teal-400/20 px-2.5 py-1.5 rounded">
          ...
          {session?.user ? (
            <div className="relative group flex items-center h-full py-2">
              {session.user.image ? (
                ...
                <img
                  ...
                  className="h-7 w-7 rounded-full border border-zinc-700 shadow shadow-teal-500/10 cursor-pointer"
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[10px] font-semibold text-zinc-950 bg-gradient-to-r from-teal-400 to-emerald-500 hover:from-teal-500 hover:to-emerald-600 px-3 py-1.5 rounded shadow-lg shadow-teal-500/20 transition-all cursor-pointer font-bold"
            >
              <FaSignInAlt />
          ...
          {navLinks.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-2.5 text-sm font-medium px-3 py-2.5 rounded transition-all active:bg-teal-500/20"
                style={{
                  color: active ? "#14b8a6" : "#d4d4d8",
          ...
          <div className="border-t border-zinc-800 pt-3 mt-1 flex flex-col gap-2">
            {session?.user ? (
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 text-sm font-semibold text-zinc-950 bg-gradient-to-r from-teal-400 to-emerald-500 px-4 py-2.5 rounded transition-all cursor-pointer font-bold"
              >
                <FaSignInAlt />
                Sign in with Google
              </button>
