...
import { FaCoins, FaUser, FaBrain, FaSignOutAlt, FaGoogle } from "react-icons/fa";
    ...
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3.5 bg-zinc-950/80 border-b border-zinc-800/50 backdrop-blur-md text-zinc-100 flex-shrink-0">
      ...
      <div className="flex items-center gap-5 sm:gap-7 min-w-0">
        <Link href="/" className="flex items-center gap-2 font-extrabold tracking-tight text-white flex-shrink-0 group">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-indigo-600 to-violet-500 flex items-center justify-center text-white shadow-lg shadow-indigo-500/20 group-hover:scale-105 transition-transform duration-200">
            <FaBrain className="text-sm" />
          </div>
          <span className="text-sm sm:text-base leading-none">
            KnowBase<span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-400 font-black">AI</span>
          </span>
        </Link>
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        ...
        <a
          href="https://vercel.com/new/clone?repository-url=https://github.com/SamurAIGPT/ai-knowledge-base"
          target="_blank"
        ...
        {session?.user ? (
          <>
            {/* Pulsing Coins Balance */}
            <span className="flex items-center gap-1.5 text-xs font-bold text-amber-300 bg-amber-955/30 border border-amber-800/40 px-3 py-1.5 rounded-full shadow-inner">
            ...
            <button
              onClick={() => signOut()}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-zinc-505 hover:text-red-400 hover:bg-red-955/20 transition-all cursor-pointer"
              title="Sign out"
            >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 active:scale-[0.98] rounded-lg shadow-lg shadow-indigo-500/20 transition-all cursor-pointer"
          >
