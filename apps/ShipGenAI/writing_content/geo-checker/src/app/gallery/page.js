...
import {
  FaSpinner, FaGoogle, FaImages, FaEye, FaPlus, FaTrashAlt, FaGlobe, FaCoins, FaCheckCircle, FaExclamationTriangle
} from "react-icons/fa";
...
export default function GalleryPage() {
  ...
  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this AI visibility report? This action cannot be undone.")) return;
    setDeletingId(id);
    try {
      ...
      <div className="flex-1 flex items-center justify-center bg-zinc-950 px-4 py-12">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center shadow-xl">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed mb-8">
            Please sign in to access your audit showroom, view detail report parameters, and download completed GEO recommendations.
          </p>
          <button
    ...
    <div className="flex-1 overflow-y-auto bg-zinc-950 text-zinc-200 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-black font-heading text-white tracking-tight">Audit Gallery</h1>
            <p className="text-xs sm:text-sm text-zinc-400 mt-1.5 font-medium font-sans">Browse your completed website AI search optimization reviews</p>
          </div>
          <Link
            href="/"
        ...
        {reports.length === 0 ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 text-center shadow-lg max-w-xl mx-auto my-12">
            <div className="h-16 w-16 bg-zinc-950 text-zinc-400 border border-zinc-800 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-inner">
              <FaGlobe className="text-3xl text-zinc-300" />
            </div>
            <h2 className="text-lg font-bold text-zinc-200 mb-2">No completed audits yet</h2>
            <p className="text-sm text-zinc-400 leading-relaxed max-w-sm mx-auto mb-8 font-medium">
              You don't have any successfully finished AI search visibility audits in your showroom yet. Audit a site in the studio!
            </p>
            <Link
              href="/"
            ...
            {reports.map((creation) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] bg-zinc-950 overflow-hidden flex flex-col items-center justify-center p-4 border-b border-zinc-800">
                    <div className="relative h-20 w-20 flex items-center justify-center">
                      <svg className="absolute inset-0 w-full h-full transform -rotate-90">
                        <circle cx="40" cy="40" r="32" className="stroke-zinc-800" strokeWidth="6" fill="transparent" />
                        <circle cx="40" cy="40" r="32" className="stroke-violet-500" strokeWidth="6" fill="transparent"
                          strokeDasharray={2 * Math.PI * 32}
                          strokeDashoffset={2 * Math.PI * 32 - (creation.score / 100) * (2 * Math.PI * 32)}
                          strokeLinecap="round"
                        />
                      </svg>
                      <div className="flex flex-col items-center justify-center z-10">
                        <span className="text-lg font-black text-white">{creation.score}%</span>
                        <span className="text-[6px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">Score</span>
                      </div>
                    </div>

                    <div className="mt-3 text-center min-w-0 px-2">
                      <div className="text-xs font-black text-zinc-200 truncate">{creation.url}</div>
                      <div className="text-[10px] text-zinc-500 truncate mt-1">"{creation.keyword}"</div>
                    </div>
                    ...
                    <span className="absolute top-3 left-3 text-[7px] font-bold text-violet-300 bg-zinc-950 border border-zinc-800 px-2 py-0.5 rounded-lg shadow uppercase">
                      GEO Report
                    </span>
                  </div>

                  {/* Card footer details */}
                  <div className="p-4 bg-zinc-900 flex items-center justify-between text-[10px] text-zinc-400 font-bold">
                    <span>
        ...
        {selectedReport && selectedReport.parsed && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md transition-opacity" onClick={() => setSelectedReport(null)}>
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-3xl w-full p-5 sm:p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200 flex flex-col overflow-hidden max-h-[95vh]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center justify-between border-b border-zinc-800 pb-3 mb-4 flex-shrink-0">
                <div>
                  <h3 className="text-sm sm:text-base font-bold font-heading text-white flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold text-violet-400 bg-violet-950 border border-violet-850 px-2 py-0.5 rounded-lg uppercase">
                      18 Credits Used
                    </span>
                  </h3>
                </div>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 space-y-5 pr-1">
                ...
                {/* Score gauge grids */}
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 bg-zinc-950 p-4 border border-zinc-800 rounded-xl items-center justify-center">
                  <div className="sm:col-span-1 flex justify-center">
                    <div className="relative h-20 w-20 flex items-center justify-center">
                      <svg className="absolute inset-0 w-full h-full transform -rotate-90">
                        <circle cx="40" cy="40" r="32" className="stroke-zinc-805" strokeWidth="6" fill="transparent" />
                        <circle cx="40" cy="40" r="32" className="stroke-violet-500" strokeWidth="6" fill="transparent"
                          strokeDasharray={2 * Math.PI * 32}
                          strokeDashoffset={2 * Math.PI * 32 - (selectedReport.score / 100) * (2 * Math.PI * 32)}
                          strokeLinecap="round"
                        />
                      </svg>
                      <div className="flex flex-col items-center justify-center z-10">
                        <span className="text-lg font-black text-white">{selectedReport.score}%</span>
                        <span className="text-[6px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">Visibility</span>
                      </div>
                    </div>
                  </div>

                  <div className="sm:col-span-3 grid grid-cols-3 gap-2">
                    <div className="bg-zinc-900/60 p-2.5 border border-zinc-800 rounded-lg text-center">
                      <div className="text-sm font-black text-violet-400">{selectedReport.parsed.eeat_score}%</div>
                      <div className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">E-E-A-T</div>
                    </div>
                    <div className="bg-zinc-900/60 p-2.5 border border-zinc-800 rounded-lg text-center">
                      <div className="text-sm font-black text-fuchsia-400">{selectedReport.parsed.citation_likelihood}%</div>
                      <div className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">Citation</div>
                    </div>
                    <div className="bg-zinc-900/60 p-2.5 border border-zinc-800 rounded-lg text-center">
                      <div className="text-sm font-black text-emerald-400">{selectedReport.parsed.readability_score}%</div>
                      <div className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">Readability</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-zinc-950/40 p-4 border border-zinc-800 rounded-xl">
                  <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-wider block mb-1">Audit Summary</span>
                  <p className="text-xs text-zinc-300 leading-relaxed font-medium">{selectedReport.parsed.summary}</p>
                </div>

                {/* Strengths & Weaknesses */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-zinc-950/20 border border-zinc-800 p-4 rounded-xl space-y-2">
                    <span className="text-[8px] font-bold text-emerald-400 uppercase tracking-wider block">AI Strengths</span>
                    <ul className="space-y-1.5">
                      {selectedReport.parsed.strengths?.map((str, idx) => (
                        <li key={idx} className="text-xs text-zinc-300 flex items-start gap-2 leading-relaxed font-medium">
                  ...
                  <div className="bg-zinc-950/20 border border-zinc-800 p-4 rounded-xl space-y-2">
                    <span className="text-[8px] font-bold text-red-400 uppercase tracking-wider block">AI Gaps</span>
                    <ul className="space-y-1.5">
                ...
                {selectedReport.parsed.technical_audit && (
                  <div className="bg-zinc-950/30 border border-zinc-800 rounded-xl overflow-hidden">
                    <div className="px-4 py-2 border-b border-zinc-800 bg-zinc-950">
                      <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-wider block">Technical Signals Audit</span>
                    </div>
                    <div className="p-3.5 space-y-1.5 text-xs font-semibold">
                      <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-zinc-800/50 items-center">
                        <span className="text-zinc-500">robots.txt</span>
                        <span className="col-span-2 text-zinc-200">{selectedReport.parsed.technical_audit.robots_txt}</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 py-1.5 border-b border-zinc-800/50 items-center">
                        <span className="text-zinc-500">schema.org</span>
                        <span className="col-span-2 text-zinc-200">{selectedReport.parsed.technical_audit.schema_markup}</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 py-1.5 items-center">
                        <span className="text-zinc-500">XML sitemap</span>
                        <span className="col-span-2 text-zinc-200">{selectedReport.parsed.technical_audit.sitemap}</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-wider block">Recommendations Roadmap</span>
                  <div className="space-y-2">
                    {selectedReport.parsed.recommendations?.map((rec, idx) => (
                      <div key={idx} className="p-3.5 bg-zinc-955 border border-zinc-800 rounded-xl flex items-start gap-3">
                        <div className={clsx(
                          "text-[8px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded text-center flex-shrink-0 mt-0.5",
                          rec.priority === "High" ? "bg-red-950/20 text-red-400 border border-red-900/30" :
                          rec.priority === "Medium" ? "bg-amber-950/20 text-amber-300 border border-amber-800/30" :
                          "bg-zinc-900 text-zinc-400 border border-zinc-800"
                        )}>
                          {rec.priority}
                        </div>
                        <div className="space-y-0.5">
                          <div className="text-xs font-black text-white">{rec.area}</div>
                          <div className="text-xs text-zinc-400 leading-relaxed font-medium">{rec.tips}</div>
                        </div>
                      </div>
                    ))}
                  </div>
