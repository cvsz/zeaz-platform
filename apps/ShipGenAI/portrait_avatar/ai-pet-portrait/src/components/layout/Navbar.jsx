...
import { FaCoins, FaUser, FaSignOutAlt, FaGoogle, FaBars, FaTimes, FaCamera } from "react-icons/fa";
import { SiVercel } from "react-icons/si";
import clsx from "clsx";

const navLinks = [
  { name: "Generate Portrait", href: "/" },
  { name: "My Creations", href: "/gallery" },
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3.5 bg-white/85 border-b border-zinc-200/80 backdrop-blur-md text-zinc-800 flex-shrink-0">
      ...
      <div className="hidden md:flex items-center gap-2 sm:gap-3 flex-shrink-0">
        {/* Deploy button */}
        <a
          href="https://vercel.com/new/clone?repository-url=https://github.com/SamurAIGPT/ai-pet-portrait"
          target="_blank"
        ...
        <div className="absolute top-full left-0 right-0 z-[200] flex flex-col p-4 bg-white border-b border-zinc-200 backdrop-blur-lg md:hidden gap-3 shadow-xl">
          ...
          <div className="flex flex-col gap-3">
            {session?.user ? (
              <div className="flex items-center justify-between px-3 py-1">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-zinc-500 hover:text-red-650 hover:bg-red-50 transition-all"
                >
                  <FaSignOutAlt className="text-xs" />
                  <span>Sign out</span>
                </button>
            ...
            <a
              ...
              onClick={() => setIsOpen(false)}
              className="flex items-center justify-center gap-1.5 w-full py-2 text-[11px] font-bold text-zinc-650 bg-zinc-50 border border-zinc-200 rounded-lg hover:bg-zinc-100"
            >
