...
export async function GET(req) {
  try {
    ...
    if (id) {
      const creation = await prisma.petPortraitCreation.findFirst({
    ...
    const creations = await prisma.petPortraitCreation.findMany({
          ...
          try {
            ...
            if (checkRes.ok) {
              ...
              if (state === "completed" || state === "succeeded") {
                ...
                if (outputUrl) {
                  return await prisma.petPortraitCreation.update({
...
export async function DELETE(req) {
  try {
    ...
    await prisma.petPortraitCreation.delete({
      where: { id }
    });
