...
export default function GalleryPage() {
  ...
  useEffect(() => {
    if (session?.user) {
      setTimeout(() => fetchCompletedCreations(), 0);
  ...
  const handleDownload = (creation) => {
    ...
    a.href = downloadUrl;
    a.download = `pet-portrait-${creation.id}.png`;
  ...
  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this pet portrait? This action cannot be undone.")) return;
    setDeletingId(id);
    try {
  ...
  if (status === "loading" || (loading && creations.length === 0)) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white text-zinc-655">
        <FaSpinner className="animate-spin text-3xl text-emerald-500 mb-4" />
        <p className="text-sm font-medium">Loading creations gallery...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-zinc-50 px-4 py-12">
        <div className="max-w-md w-full bg-white border border-zinc-200 rounded-2xl p-8 text-center shadow-xl">
          <div className="h-14 w-14 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 flex items-center justify-center mx-auto mb-6 shadow-inner animate-pulse">
            <FaImages className="text-2xl" />
          </div>
          <h1 className="text-2xl font-bold text-zinc-900 tracking-tight mb-2">Showroom Gallery</h1>
          <p className="text-sm text-zinc-550 leading-relaxed mb-8">
            Please sign in to access your HD gallery and download completed pet portraits.
          </p>
          <button
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-zinc-50 text-zinc-850 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {creations.length === 0 ? (
          <div className="bg-white border border-zinc-200 rounded-2xl p-12 text-center shadow-lg max-w-xl mx-auto my-12">
            ...
            <h2 className="text-lg font-bold text-zinc-800 mb-2">No completed portraits yet</h2>
            <p className="text-sm text-zinc-500 leading-relaxed max-w-sm mx-auto mb-8 font-medium">
              You don&apos;t have any successfully finished pet portraits in your showroom yet. Generate one in the studio!
            </p>
            <Link
              href="/"
              ...
              >
                ...
                <div className="relative aspect-[4/5] bg-zinc-100 overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 bg-black/35 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                    <div className="h-10 w-10 bg-white border border-zinc-250 rounded-full flex items-center justify-center text-zinc-805 shadow-md">
                      <FaEye />
                    </div>
                  </div>

                  {/* Floating parameters badge */}
                  <span className="absolute top-3 left-3 text-[8px] font-bold text-emerald-650 bg-white border border-zinc-200 px-2 py-0.5 rounded-lg shadow uppercase">
                    PET ART
                  </span>
                </div>

                {/* Card footer details */}
                <div className="p-4 bg-white border-t border-zinc-200 flex items-center justify-between text-[10px] text-zinc-550 font-bold">
                  <span>
        ...
        {selectedCreation && (() => {
          let modalImages = [];
          try {
            modalImages = JSON.parse(selectedCreation.petImages || "[]");
          } catch (e) {
            if (selectedCreation.petImages) modalImages = [selectedCreation.petImages];
            else if (selectedCreation.petImage) modalImages = [selectedCreation.petImage];
          }
              ...
              <div className="bg-white border border-zinc-200 rounded-2xl max-w-2xl w-full p-5 sm:p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200 flex flex-col overflow-hidden max-h-[95vh]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between border-b border-zinc-200 pb-3 mb-4 flex-shrink-0">
                  <div>
                    <h3 className="text-sm sm:text-base font-bold text-zinc-900 flex items-center gap-2">
                      <span>Pet Portrait Art Details</span>
                ...
                <div className="flex-1 flex flex-col md:flex-row gap-6 overflow-y-auto min-h-0 items-center justify-center">
                  ...
                  <div className="relative w-full max-w-[340px] aspect-[4/5] rounded-xl overflow-hidden border border-zinc-200 bg-zinc-100 shadow-md flex-shrink-0">
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm border border-zinc-200 text-emerald-650 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-lg">
                      Applied Art Overlay
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {modalImages.map((imgUrl, idx) => (
                          <div key={idx} className="aspect-square rounded-lg overflow-hidden border border-zinc-200 bg-zinc-100">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={imgUrl} alt={`Pet Input ${idx + 1}`} className="w-full h-full object-cover" />
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="border-t border-zinc-200 pt-4 mt-4 flex justify-between items-center gap-3 flex-shrink-0">
                  <button
                    ...
                    className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-650 border border-red-200 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5"
                  >
                    <FaTrashAlt className="text-[10px]" /> Delete Art
                  </button>
                  
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleDownload(selectedCreation)}
                      className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white rounded-lg text-xs font-bold shadow-lg shadow-emerald-500/10 transition-all cursor-pointer flex items-center gap-1.5 hover:scale-[1.02]"
                    >
