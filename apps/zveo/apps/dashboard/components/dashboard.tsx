...
export function JobTimeline({ detail }: { detail: WorkflowDetail }) {
  return <ul className="space-y-2">{detail.jobs.map((j) => <li key={j.id} className="rounded border border-slate-700 p-3">{j.id} • {j.state} {j.sceneId ? `• scene ${j.sceneId}` : ""}</li>)}</ul>;
}
export function AssetPreviewList({ detail }: { detail: WorkflowDetail }) {
  return <ul className="space-y-2">{detail.assets.map((a) => <li key={a.id} className="rounded border border-slate-700 p-3">{a.kind} • {a.objectKey} {a.metadata.sceneId ? `• scene ${String(a.metadata.sceneId)}` : ""}</li>)}</ul>;
}
