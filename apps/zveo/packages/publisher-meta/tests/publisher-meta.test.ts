...
test('publishes and records transitions', async () => {
  ...
  const target = service.createTarget({ tenantId: '00000000-0000-4000-8000-000000000001', provider: 'facebook', pageId: '123', displayName: 'Page', encryptedAccessTokenRef: 'kms://ref' });
  const job = service.createJob({ tenantId: target.tenantId, workflowId: '00000000-0000-4000-8000-000000000002', assetId: '00000000-0000-4000-8000-000000000003', targetId: target.id, caption: 'hello', correlationId: '00000000-0000-4000-8000-000000000004' });
  const done = await service.publish(job.id, 'https://example/video.mp4', 'token', '00000000-0000-4000-8000-000000000004');
