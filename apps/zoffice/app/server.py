...
def _normalize_presence_entry(entry):
    ...
    stale_limit_sec = 180 if (
        ...
        bool(task_tokens & {"reading", "processing", "thinking", "running", "command", "editing", "writing", "searching", "fetching"})
    ) else 45
...
def _load_vo_config():
    ...
    return {
        "office": {
            ...
            "port": _safe_int(_env_or("VO_PORT", office.get("port", 18090)), 18090),
            "wsPort": _safe_int(_env_or("VO_WS_PORT", office.get("wsPort", 18091)), 18091),
        },
        "openclaw": {
            "homePath": oc_home,
        ...
        "presence": {
            ...
            "inferenceIdleTimeoutSec": _safe_int(presence.get("inferenceIdleTimeoutSec"), 300),
        },
        "features": {
        ...
        "hermes": {
            ...
            "timeoutSec": _safe_int(_env_or("VO_HERMES_TIMEOUT_SEC", hermes_cfg.get("timeoutSec", 600)), 600),
        },
    }
...
JSON_LOCKS = {
    "workspaces": threading.Lock(),
    "office_config": threading.Lock(),
...
def _load_comm_history(limit=200, conversation_id=None, agent_id=None):
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            # For JSONL, the most efficient way to get the last N matches 
            # without reading everything into memory is reading in chunks from the end.
            # But for simplicity and given the expected log size, we use a deque.
            from collections import deque
            # We use a larger buffer for the deque to account for filtering
            filter_buffer = deque(maxlen=max(1, int(limit or 200) * 5))
            for line in f:
                try:
...
def _run_async_blocking(coro, timeout=30):
    ...
    # Use a larger worker pool to avoid bottlenecking Gateway RPC calls.
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as pool:
        future = pool.submit(asyncio.run, coro)
        return future.result(timeout=timeout)

_GATEWAY_CONNECTION_POOL = {}
_POOL_LOCK = threading.Lock()

async def _get_gateway_ws():
    """Return a cached WebSocket connection to the Gateway if valid, else create one."""
    ...
    # Use token as key to allow multiple accounts if needed
    pool_key = f"{gw_url}:{token}"
    
    async with asyncio.Lock(): # Note: this is a local lock for the async context
        ws = _GATEWAY_CONNECTION_POOL.get(pool_key)
        ...
        try:
            ...
            connect_id = f"vo-admin-pool-{uuid.uuid4()}"
            await ws.send(json.dumps({
                ...
                "params": {
                    ...
                    "client": {"id": "vo-server-pool", "version": _get_openclaw_version(), "platform": "server", "mode": "webchat"},
                    "role": "operator", "scopes": ["operator.read", "operator.write", "operator.admin"],
                    "caps": [], "commands": [], "permissions": {}, "auth": {"token": token},
                    "locale": "en-US", "userAgent": "virtual-office-server/pool",
                },
            }))
            ...
            while True:
                ...
                if msg.get("id") == connect_id:
                    if not msg.get("ok"):
                        ...
                        return None
                    break
            _GATEWAY_CONNECTION_POOL[pool_key] = ws
            return ws
        except Exception:
            return None
...
async def _gateway_rpc_call_async(method, params=None, timeout=20):
    """Call an OpenClaw Gateway RPC using a pooled WebSocket connection."""
    ws = await _get_gateway_ws()
    if not ws:
        return {"ok": False, "error": "Failed to connect to Gateway or token missing"}
    ...
    try:
        ...
        while time.time() < deadline:
            msg_raw = await asyncio.wait_for(ws.recv(), timeout=min(10, max(1, deadline - time.time())))
        ...
        return {"ok": False, "error": f"RPC call failed: {str(e)}"}
...
def _signal_gateway_reload():
    ...
    try:
        # Find gateway PID from proc (with overall timeout)
...
class OfficeHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/setup":
            ...
            refresh_agent_maps()
            result = {}
            for agent_key in dict(AGENT_SESSION_IDS):
