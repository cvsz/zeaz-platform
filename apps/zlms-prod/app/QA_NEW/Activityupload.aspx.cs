  ...
  public class Activityupload : Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      foreach (string file1 in (NameObjectCollectionBase) this.Request.Files)
      {
        ...
        try
        {
          SafeUploadResult upload = FileUploadSecurity.Save(file2, "~/Activities/", str1, string.Empty);
    ...
    private void addUser(string filename, string filepath)
    {
      ...
      cconnect.sqlCmdText("INSERT INTO QA_activities_file ([ActivitiesID],[FileName],[FilePath],[userid],[CreatedDate],[Updatedate],[Updateby],Active) VALUES(@ownerId,@filename,@filepath,@userid,@createdDate,@updatedDate,@updateBy,@active)");
