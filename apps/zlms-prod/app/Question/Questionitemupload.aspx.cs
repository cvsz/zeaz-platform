  ...
  public class Questionitemupload : Page
  {
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
      ...
      if (this.FileUpload1.HasFile)
      {
        try
        {
          SafeUploadResult upload = FileUploadSecurity.Save(this.FileUpload1.PostedFile, "~/Questionpic/", str1, string.Empty);
          this.Label1.Text = "File name: " + HttpUtility.HtmlEncode(upload.FileName) + "<br>" + (object) this.FileUpload1.PostedFile.ContentLength + " bytes<br>Content type: " + HttpUtility.HtmlEncode(this.FileUpload1.PostedFile.ContentType);
          this.addUser(upload.FileName, upload.FullPath);
        }
        catch (Exception ex)
        {
                AppLogger.FormError(this, System.Reflection.MethodBase.GetCurrentMethod().Name, ex);
          SecurityTelemetry.Warn("question.upload", AppLogger.Redact(ex.Message), string.Empty);
          this.Label1.Text = "ERROR: upload rejected by security policy.";
        }
      }
      else
    ...
    private void addUser(string filename, string filepath)
    {
      ...
      cconnect.sqlCmdText("update Question_data set ispic=@ispic,piclocation=@piclocation,filename=@filename where id=@id");
      cconnect.sqlCmdAddParam("@ispic", 1);
      cconnect.sqlCmdAddParam("@piclocation", filepath);
      cconnect.sqlCmdAddParam("@filename", filename);
      cconnect.sqlCmdAddParam("@id", str);
      cconnect.sqlCmd();
    }
