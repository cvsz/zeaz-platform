...
- **Unified Datastore**:
  - PostgreSQL with Prisma ORM is the primary database.
...
│   │       ├── models/        # Pydantic & Prisma model helpers
...
### A. Unified Database Schema (Prisma)
The database must capture both business logic (rental subscriptions) and execution logic (orders, audit events).

```prisma
// Example segment of apps/ztrader/backend/prisma/schema.prisma
...
model ExchangeKey {
  ...
  encryptedSecret String       // AES-GCM encrypted
  passphrase    String?        // AES-GCM encrypted
}
...
model AuditLog {
  id            String         @id @default(uuid())
  eventType     String         // risk_denied, strategy_intent, order_filled
  actor         String         // strategy_id or user_id
...
gantt
    ...
    DB Schema & Prisma Integration   :        des2, after des1, 4d
...
- **Stage 2**: Establish database tables via Prisma. Port the `RiskEngine`, `PaperBroker`, `LiveBroker` (CCXT), and Celery worker.
