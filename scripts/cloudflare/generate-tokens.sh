...
notice(){
  cat >&2 <<NOTICE
[deprecated] scripts/cloudflare/generate-tokens.sh is a compatibility wrapper.
[deprecated] Legacy email/key authentication is disabled.
...
Environment compatibility:
  ENV_FILE may be used as --write when --write is not supplied.
USAGE
}
...
if ! contains_arg --write "${args[@]}"; then
  args+=(--write "${ENV_FILE:-$DEFAULT_OUT}")
fi
