...
tmp="$(mktemp "${TMPDIR:-/tmp}/zeaz-verify.XXXXXX")"
placeholder_scan="$(mktemp "${TMPDIR:-/tmp}/zeaz-placeholder-scan.XXXXXX")"
cleanup(){ rm -f "$tmp" "$placeholder_scan"; }
trap cleanup EXIT
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  add pass docker_compose "$(docker compose version --short)"
else
  add warn docker_compose missing
fi
...
python3 - > "$placeholder_scan" <<'PYSCAN'
...
rx = re.compile('|'.join(re.escape(term) for term in terms), re.I)
hits = []
...
for root in roots:
    ...
    for path in paths:
        if not path.is_file() or path.name == 'verify-all.sh':
            continue
        try:
...
if [ -s "$placeholder_scan" ]; then
  add fail placeholder_scan "unsafe placeholders found in platform-owned files"
else
  add pass placeholder_scan clean
fi
...
while IFS='|' read -r app domain port dir; do
  if command -v curl >/dev/null 2>&1; then
    ...
    if [ "$skip_domain" -ne 1 ] && [ "$domain" != internal ]; then
      curl -fsSI -m 5 "https://$domain" >/dev/null 2>&1 && add pass "domain_$app" "$domain" || add warn "domain_$app" "$domain unreachable"
    fi
  fi
done < <(scripts/ports/list-all-ports.sh --plain)

python3 - "$tmp" "$json" "$pass" "$warn" "$fail" <<'PY'
import datetime
import json
import sys
...
out = {
    ...
    'summary': {'pass': int(sys.argv[3]), 'warn': int(sys.argv[4]), 'fail': int(sys.argv[5])},
    'checks': rows,
}
