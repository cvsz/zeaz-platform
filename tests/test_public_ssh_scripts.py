...
def test_cloudflare_route_script_has_manual_mode_and_preserves_ingress():
    ...
    assert "CLOUDFLARE_API_TOKEN CLOUDFLARE_ACCOUNT_ID CF_TUNNEL_ID" in content
...
def test_cloudflare_route_manual_mode_does_not_require_credentials_or_mutate_api():
    env = os.environ.copy()
    for key in ("CLOUDFLARE_API_TOKEN", "CLOUDFLARE_ACCOUNT_ID", "CF_TUNNEL_ID"):
        env.pop(key, None)
    ...
    assert "[WARN] Missing API environment variables: CLOUDFLARE_API_TOKEN,CLOUDFLARE_ACCOUNT_ID,CF_TUNNEL_ID" in result.stdout
