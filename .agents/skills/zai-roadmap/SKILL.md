...
**Read `.ai-factory/skill-context/zai-roadmap/SKILL.md`** — MANDATORY if the file exists.
...
- Each milestone is a **high-level goal**, not a granular task (that's `/zai-plan`)
...
/zai-plan <milestone description>  → creates a plan and optional branch/worktree flow
/zai-implement                     → executes the plan
```

---

### Mode 3: Check Progress (`/zai-roadmap check`)

Automated scan — analyze the codebase and mark completed milestones without interactive questions.

**Requires** the resolved roadmap path to exist. If it doesn't — tell the user to run `/zai-roadmap` first.
...
5. **NO implementation** — this skill only plans, use `/zai-plan` to start a feature and `/zai-implement` to execute
6. **Ownership boundary** — this command owns roadmap structure/content; `/zai-implement` may only mark milestones completed when implementation evidence is clear
