...
**Read `.ai-factory/skill-context/zai-skill-generator/SKILL.md`** — MANDATORY if the file exists.
...
0. Scope check (MANDATORY):
   ...
   - If path points to built-in AI Factory skills ({{skills_dir}}/aif or {{skills_dir}}/zai-*), this is wrong target selection for install-time security checks.
...
2. LEVEL 1 — Run automated scan:
   `python3 ~/{{skills_dir}}/zai-skill-generator/scripts/security-scan.py <skill-path>` when `PYTHON_CMD=(python3)`.
...
5. If BLOCKED at any level → run the cleanup helper with the same selected Python 3 command, for example `python3 ~/{{skills_dir}}/zai-skill-generator/scripts/cleanup-blocked-skill.py --skill <name> --installed-path <skill-path>` (reuse the same `<skill-path>` you passed to security-scan.py — upstream `skills` sanitizes the directory name on disk, so synthesizing `{{skills_dir}}/<name>` can miss the real folder; `--installed-path` lets the helper verify physical removal), report threats to user
```
...
- `/zai-skill-generator <name>` - Generate a new skill interactively
- `/zai-skill-generator <url> [url2] [url3]...` - **Learn Mode**: study URLs and generate a skill from them
- `/zai-skill-generator search <query>` - Search existing skills on skills.sh for inspiration
- `/zai-skill-generator scan <path>` - **Security scan**: run two-level security check on a skill
- `/zai-skill-generator validate <path>` - **Full validation**: structure check + two-level security scan
- `/zai-skill-generator template <type>` - Get a template (basic, task, reference, visual)
...
**Trigger:** `/zai-skill-generator scan <path>`
...
3. **LEVEL 1** — Run automated scanner only when `PYTHON_CMD` is set:
   ...
   python3 ~/{{skills_dir}}/zai-skill-generator/scripts/security-scan.py <path>
   ```
...
**Trigger:** `/zai-skill-generator validate <path>`
...
7. **AUTO-SCAN**: Run `/zai-skill-generator scan <generated-skill-path>` on the result
...
python3 ~/{{skills_dir}}/zai-skill-generator/scripts/security-scan.py <installed-path>
```
If BLOCKED → run the selected concrete Python command with `~/{{skills_dir}}/zai-skill-generator/scripts/cleanup-blocked-skill.py --skill <name> --installed-path <installed-path>` (reuse the same `<installed-path>` you passed to security-scan.py — upstream `skills` sanitizes the directory name, so synthesizing `{{skills_dir}}/<name>` can miss the real folder; `--installed-path` lets the helper verify physical removal), warn. If WARNINGS → show to user.
...
```bash
# Example for PYTHON_CMD=(python3).
python3 ~/{{skills_dir}}/zai-skill-generator/scripts/security-scan.py ./skill-name/
```
