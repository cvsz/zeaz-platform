...
1. Prepare sources.
   ...
   - For large folders or PDFs, use `{{skills_dir}}/zai-distillation/scripts/material-prep.py` only when a working Python 3 interpreter is available. Detect it with `python3 --version`, `python --version`, `py -3 --version`, then `py --version`; use the first command that exits successfully and reports Python major version 3.
...
6. Validate usefulness.
   ...
   - Generated quality-gate skills must follow the `aif-gate-result` contract from `/zai-verify` references.
