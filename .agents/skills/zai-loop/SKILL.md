...
**Read `.ai-factory/skill-context/zai-loop/SKILL.md`** — MANDATORY if the file exists.
...
5. Suggest next skills based on artifact type:
   - API spec -> `/zai-plan` to implement it
   - Code -> `/zai-verify` to check it
   - Docs -> `/zai-docs` to integrate it
...
```text
💡 Context is growing. Recommended: /clear then /zai-loop resume
...
```text
/zai-loop new OpenAPI 3.1 spec + DDD notes + JSON examples
/zai-loop resume
/zai-loop resume courses-api-ddd
/zai-loop status
/zai-loop stop
/zai-loop list
/zai-loop history
/zai-loop history courses-api-ddd
/zai-loop clean courses-api-ddd
/zai-loop clean --all
```
