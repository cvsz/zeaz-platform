---
name: zai-improve
description: Refine an existing implementation plan with a second iteration. Re-analyzes the codebase for gaps, missing tasks, and wrong dependencies. Use after /zai-plan or to improve an /zai-fix plan. Optional +check flag validates refinements via a fresh-context subagent.
...
2. No explicit `@<path>` override → Check current git branch:
   ...
   → Otherwise look for `<configured plans dir>/<branch-slug>.md` (from /zai-plan full)
...
4. No full-mode plan → Check the resolved fast plan path (from /zai-plan fast)
5. No full-mode plan and no resolved fast plan → Check the resolved fix plan path (from /zai-fix plan mode)
```
...
- /zai-plan full <description>  — for a new feature (rich full plan; may also create a branch when git settings allow it)
- /zai-plan fast <description>  — for a quick task plan
- /zai-fix <bug description>    - for a bugfix plan (use the resolved fix plan path)
```
...
- If `.ai-factory/skill-context/zai-improve/SKILL.md` does not exist and the resolved patches dir exists:
...
**Read `.ai-factory/skill-context/zai-improve/SKILL.md`** — MANDATORY if the file exists.
...
Now do a **deeper** codebase exploration than what `/zai-plan` did initially:
