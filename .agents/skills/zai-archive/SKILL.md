...
Read `.ai-factory/skill-context/zai-archive/SKILL.md` if it exists —
...
5. **Do not modify fast plans** (`paths.plan`) or fix plans (`paths.fix_plan`) —
   those are single-file artifacts managed by `/zai-implement` and `/zai-fix`
6. **Do not count archived plans for sequential numbering** — archived plans
   live in `paths.archive/plans/`, not `paths.plans/`, so `/zai-plan`
