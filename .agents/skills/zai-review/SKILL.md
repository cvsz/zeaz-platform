...
If the user wants a standalone rules-only pass, suggest `/zai-rules-check`. Keep human `/zai-review` gate labels at `WARN` / `ERROR`, then append the standard machine-readable gate result with `pass|warn|fail` status.
...
- `"suggested_next.command"` follows `"status"`: `fail` → `/zai-fix` by default, but if every blocker came from a single context gate point at that gate's command instead (rules gate → `/zai-rules`, architecture gate → `/zai-architecture`, roadmap gate → `/zai-roadmap`); `warn`/`pass` → `/zai-commit`; `null` only when no command fits.

`/zai-review` is read-only for context artifacts by default. Do not modify context files unless user explicitly asks.

### Project Context

**Read `.ai-factory/skill-context/zai-review/SKILL.md`** — MANDATORY if the file exists.
...
**User:** `/zai-review https://github.com/org/repo/pull/123`
Review PR from URL.

**User:** `/zai-review 2.x`
Review all commits on the current branch compared to branch `2.x`.

**User:** `/zai-review main`
