...
description: >-
  ...
  Use after "/zai-implement" completes, or when user says "verify", "check work", "did we miss anything".
...
Same logic as `/zai-implement` — produce the **canonical branch stem** before any plans-dir glob so producer and consumers agree by construction.
...
4. If the branch-based plan is missing or git mode is off:
   ...
   → If multiple exist, ask the user to choose or use `@<path>` via `/zai-implement`
...
**Note:** Plan discovery scans `paths.plans/` only. Plans archived to `paths.archive/plans/` by `/zai-archive` are excluded from discovery. If a plan is found only in the archive, emit `WARN [aif-verify] plan <name> is archived; verifying archived plan`.
...
**Read `.ai-factory/skill-context/zai-verify/SKILL.md`** — MANDATORY if the file exists.
...
If the user wants a standalone rules-only pass, suggest `/zai-rules-check`. Keep human context-gate labels at `WARN` / `ERROR`, then derive the final machine-readable gate result from the full verification report.
...
- Set `"suggested_next": {` to `/zai-fix`, `/zai-rules`, `/zai-architecture`, `/zai-roadmap`, `/zai-commit`, or `null` according to `references/GATE-RESULT-CONTRACT.md`.

### 3.6 Context Drift (Optional Remediation)

`/zai-verify` is **read-only** for context artifacts. Do not edit or regenerate `.ai-factory/*` files here.
...
- `DESCRIPTION.md` drift → suggest `/aif` (or note that `/zai-implement` should have updated it during implementation)
- `ARCHITECTURE.md` drift → suggest `/zai-architecture`
- `ROADMAP.md` drift → suggest `/zai-roadmap check` (or `/zai-roadmap <update request>`)
- `RULES.md` drift → suggest `/zai-rules <rule text>`
...
Options:
1. Fix now (recommended) — Use /zai-fix to address all issues
2. Fix critical only — Use /zai-fix for incomplete tasks, skip warnings
3. Fix directly here — Address issues in this session without /zai-fix
4. Accept as-is — Mark everything as done, move on
```

**If "Fix now" or "Fix critical only":**
- First suggest using `/zai-fix` and pass a concise issue summary as argument
- Example:
  - `/zai-fix complete Task #3 password reset email flow, implement Task #8 docs update, remove unfinished markers in src/services/auth.ts and src/middleware/rate-limit.ts, document SENDGRID_API_KEY in .env.example`
- If user agrees, proceed via `/zai-fix`
- If user declines `/zai-fix`, continue with direct implementation in this session
- For each incomplete/partial task — implement the missing pieces (follow the same implementation rules as `/zai-implement`)
...
- If unresolved issues remain (accepted or deferred), suggest `/zai-fix` first
...
1. 🛠️ /zai-fix [issue summary] — Fix remaining verification issues
2. 🔒 /zai-security-checklist — Run security audit on the new code
3. 👀 /zai-review — Code review of the implementation
4. 💾 /zai-commit — Commit the changes
...
Options:
1. Fix issues — Run /zai-fix with verification findings
2. Security check — Run /zai-security-checklist on changed files
3. Code review — Run /zai-review on the implementation
4. Both — Run security check, then code review
5. Skip — Proceed to commit
```

**If fix issues selected** → suggest invoking `/zai-fix <issue summary>`
**If security check selected** → suggest invoking `/zai-security-checklist`
**If code review selected** → suggest invoking `/zai-review`
**If both** → suggest security first, then review
**If skip** → suggest `/zai-commit`
