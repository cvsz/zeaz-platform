---
name: zai-prompt
description: Advanced techniques for crafting effective prompts for Large Language Models.
---
...
## 1. Overview
Prompt Engineering is the practice of designing inputs to effectively guide AI models toward desired behaviors and outputs.

## 2. Core Prompting Techniques
* **Zero-Shot Prompting**: Asking the model to perform a task without providing any examples.
* **Few-Shot Prompting**: Providing 2-5 examples of the desired input-output pair to establish a pattern.
* **Chain-of-Thought (CoT)**: Instructing the model to "think step-by-step" to improve reasoning in complex logic or math problems.
* **Role-Playing**: Assigning the model a specific persona (e.g., "Act as a Senior Python Developer").

## 3. Best Practices for Prompt Design
* **Be Specific and Explicit**: Do not assume the model knows your constraints. Explicitly state format, length, and tone (e.g., "Output strictly as JSON", "Max 50 words").
* **Use Delimiters**: Separate instructions from context data using XML tags or markdown delimiters (e.g., `<context>...</context>`).
* **Positive Framing**: Tell the model what TO do, rather than what NOT to do, whenever possible.
* **Provide an Output Template**: Show the model exactly how you want the final response structured.

## 4. Anti-Patterns to Avoid
* **Vague Instructions**: e.g., "Write a good blog post."
* **Overloading**: Giving the model 15 different conflicting tasks in a single prompt. Break complex tasks into a multi-prompt pipeline.


## Sub-skill: prompt-optimizer
