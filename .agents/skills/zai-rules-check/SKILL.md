...
**Read `.ai-factory/skill-context/zai-rules-check/SKILL.md`** - MANDATORY if the file exists.
...
1. Compute the **canonical branch stem** the same way as `/zai-plan`,
   `/zai-implement`, and `/zai-improve`:
...
- Suggest `/zai-rules <rule text>` for axioms
- Suggest `/zai-rules area:<name>` for area-specific rules
...
- `/zai-review` for broader code review
- `/zai-verify` for full plan-completeness verification
- `/zai-rules` when the underlying rules need to be captured or corrected
...
- Set `"suggested_next": {` to `/zai-rules` when rules should be added or clarified, `/zai-fix` when code must change, or `null` when no allowed next command fits.
- Do not use `/zai-review` in the JSON `suggested_next.command`; it may appear only in human-readable workflow suggestions.
