...
- `/zai-security-checklist` — Full audit checklist
- `/zai-security-checklist auth` — Authentication & sessions
- `/zai-security-checklist injection` — SQL/NoSQL/Command injection
- `/zai-security-checklist xss` — Cross-site scripting
- `/zai-security-checklist csrf` — Cross-site request forgery
- `/zai-security-checklist secrets` — Secrets & credentials
- `/zai-security-checklist api` — API security
- `/zai-security-checklist infra` — Infrastructure security
- `/zai-security-checklist prompt-injection` — LLM prompt injection
- `/zai-security-checklist race-condition` — Race conditions & TOCTOU
- `/zai-security-checklist ignore <item>` — Ignore a specific check item
...
**When the user runs `/zai-security-checklist ignore <item>`:**
...
**When running any audit (`/zai-security-checklist` or a specific category):**
...
└─────────────────┴──────────────────────────────────────┴────────────┘
⚠️  2 items ignored. Run `/zai-security-checklist` without ignores to see full audit.
```
...
**Read `.ai-factory/skill-context/zai-security-checklist/SKILL.md`** — MANDATORY if the file exists.
...
For `/zai-security-checklist` audits (full audit or category audit), keep the human-readable security report first and append one final fenced `aif-gate-result` JSON block.
...
- Set `"suggested_next": {` to `/zai-fix` for code/config security fixes or `null` when no workflow command fits.
