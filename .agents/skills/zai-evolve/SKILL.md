...
- `/zai-evolve` is the primary raw-patch analyzer. It processes patches **incrementally** using a cursor.
- `/zai-implement`, `/zai-fix`, and `/zai-improve` should prefer skill-context first; raw patches are fallback context only.
- Force full re-analysis only when needed (e.g., reset cursor and rerun evolve).

## Critical: Never Edit Built-in Skills Directly

**NEVER modify any files inside built-in `aif-*` skill directories** (`skills/zai-*/`).
...
"Skill '<resolved-name>' not found. Use `/zai-evolve` without arguments to evolve
...
- If evolving a **specific skill** (e.g., `/zai-evolve plan`) → read only:
  1. `.ai-factory/skill-context/zai-plan/SKILL.md` (target skill's context)
  2. `.ai-factory/skill-context/zai-evolve/SKILL.md` (evolve's own context, if exists
     **and** target skill is not `aif-evolve` itself)
- If evolving **all skills** (`/zai-evolve` or `/zai-evolve all`) → read all context files:
...
7. Full rescan procedure: delete `<resolved evolutions dir>/patch-cursor.json`, then run `/zai-evolve` again.
...
(e.g., for `/zai-plan` — focus on file structure and naming; for `/zai-fix` — error handling and testing).

### Step 2: Read Target Skills

**Read ONLY the base SKILL.md files for target skills — not all skills.**

- If evolving a **specific skill** (e.g., `/zai-evolve plan`) → read only that one:
  `Read: {{skills_dir}}/zai-plan/SKILL.md` (or `skills/zai-plan/SKILL.md` if not installed)
- If evolving **all skills** (`/zai-evolve` or `/zai-evolve all`) → read all:
...
It must NEVER propose editing, deleting, or reverting base `skills/zai-*/` files.
...
Never propose editing, deleting, or reverting base `skills/zai-*/` files —
...
##### /zai-plan — Fully covered: [Rule Name]
...
##### /zai-fix — Partial overlap: [Rule Name]
- **Base SKILL.md says:** [base rule text]
- **Skill-context says:** [project rule text]
...
#### /zai-fix (N rules)
**Target:** `.ai-factory/skill-context/zai-fix/SKILL.md`
...
#### /zai-implement (N rules)
**Target:** `.ai-factory/skill-context/zai-implement/SKILL.md`
...
5. **NEVER edit any files inside the skill's `skills/zai-*/` directory** — all files there are owned by ai-factory and WILL be overwritten on update. All improvements go to skill-context only.
...
> Auto-generated by `/zai-evolve`. Do not edit manually.
...
1. **Run `/zai-review`** on recent code to verify improvements
2. **Next evolution** — run `/zai-evolve` again after 5-10 more fixes
3. **Consider new skill** — if pattern X keeps recurring, create a dedicated skill:
   `/zai-skill-generator <skill-name>`
```
...
7. **Skill-context only** — all improvements for built-in `aif-*` skills go to `.ai-factory/skill-context/`, never to `skills/zai-*/`. **NEVER edit any files inside `skills/zai-*/`** — they are overwritten on update. No exceptions.
...
10. **No new skills** — suggest `/zai-skill-generator` instead
...
→ Improvement for /zai-fix (2 rules):
  Target: .ai-factory/skill-context/zai-fix/SKILL.md
