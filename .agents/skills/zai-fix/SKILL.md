...
- **Immediately check the first line for `<!-- handoff:task:<uuid> -->`:**
  - If found AND `HANDOFF_MODE` is NOT `1` (manual session): extract the task ID. Call `handoff_sync_status` with `{ taskId: <extracted-id>, newStatus: "implementing", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`. (Status is `"implementing"` because we are executing an existing plan, not creating one.)
...
- Tell the user: "No fix plan found and no problem description provided. Please either provide a bug description (`/zai-fix <description>`) or create a fix plan first."
- **STOP.**
...
**Read `.ai-factory/skill-context/zai-fix/SKILL.md`** — MANDATORY if the file exists.
...
- If `.ai-factory/skill-context/zai-fix/SKILL.md` does not exist and the resolved patches dir exists:
...
**Handoff sync (manual mode only):** If a Handoff task ID is known (from `HANDOFF_TASK_ID` or an existing annotation) AND `HANDOFF_MODE` is NOT `1`, call `handoff_sync_status` with `{ taskId: <id>, newStatus: "planning", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.
...
**Handoff sync (manual mode only):** If a Handoff task ID is known AND `HANDOFF_MODE` is NOT `1`, call `handoff_push_plan` with `{ taskId: <id>, planContent: <full fix plan text> }`, then `handoff_sync_status` with `{ taskId: <id>, newStatus: "plan_ready", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.

**STOP here. Do NOT apply the fix.**

### Step 2: Investigate the Codebase

**Handoff sync (manual mode, "Fix now" path only):** If a Handoff task ID is known AND `HANDOFF_MODE` is NOT `1`, call `handoff_sync_status` with `{ taskId: <id>, newStatus: "implementing", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.
...
**User:** `/zai-fix TypeError: Cannot read property 'name' of undefined in UserProfile`
...
**User:** `/zai-fix /api/orders returns empty array for authenticated users`
...
**User:** `/zai-fix email validation accepts invalid emails`
...
11. **Ownership boundary** - `/zai-fix` owns `paths.fix_plan` and `paths.patches`; treat `.ai-factory/DESCRIPTION.md`, roadmap, rules, and architecture context artifacts as read-only unless the user explicitly requests otherwise
