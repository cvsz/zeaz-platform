...
- `skills/zai-implement/references/IMPLEMENTATION-GUIDE.md` → "List Available Plans (`--list`)"

### Step 0.inline: Inline Implementation Mode (`--without-plan`)

If `$ARGUMENTS` contains `--without-plan`, execute a single scoped task from the description WITHOUT creating or reading any plan file. This is the lightweight path for small `feat`/`chore` tasks that do not justify a full plan but are not bug fixes either (use `/zai-fix` for bugs).
...
3. Validation:
   - description is empty →
     ERROR: "Usage: /zai-implement --without-plan <description> [--docs=yes|no|warn]"
     → STOP
...
- `.ai-factory/skill-context/zai-implement/SKILL.md` — MANDATORY if the file exists (same precedence and enforcement as regular mode in Step 0.1)
- `language.ui`, `language.artifacts`
...
5. Do not add tests by default. Add them only if the description explicitly requests tests (e.g. "with tests", "add tests for X") OR if existing project conventions / touched code paths clearly require them (e.g. a test file mirrors every source file in the area being changed, or a RULES.md / skill-context rule mandates test coverage for this kind of change). When in doubt, prefer NO tests and let the user follow up via `/zai-plan` if wider test coverage is needed.
...
- Do NOT invoke `/zai-plan` or `/zai-fix`.
...
2. Call `handoff_sync_status` with `{ taskId: <HANDOFF_TASK_ID>, newStatus: "implementing", sourceTimestamp: "<current UTC ISO 8601>", direction: "zai_to_handoff", paused: true }`.
...
- `--docs=yes` → after completion, show the docs checkpoint (same AskUserQuestion as `Docs: yes` in regular mode) and route changes through `/zai-docs`.
...
1. 🔍 /zai-verify — Verify the change (recommended)
2. 💾 /zai-commit — Commit directly
```
...
Options:
1. Verify first — Run /zai-verify (recommended)
2. Skip to commit — Go straight to commit (make gpg-finalize)
```
...
Options:
1. Yes, commit now (make gpg-finalize)
...
- Yes → run `/zai-commit`, then continue to plan discovery
- No → `git stash push -m "zai-implement: stash before plan execution"`, then continue
- Cancel → inform the user: "Implementation cancelled." → **STOP**

**If NO plan file exists but the resolved fix plan exists:**

A fix plan was created by `/zai-fix` in plan mode. Redirect to fix workflow:

```
Found a fix plan at the resolved fix plan path.

This plan was created by /zai-fix and should be executed through the fix workflow
(it creates a patch and handles cleanup automatically).

Running /zai-fix to execute the plan...
```

→ **Invoke `/zai-fix`** (without arguments – it will detect the resolved fix plan and execute it).
...
- New feature from current → `/zai-plan full <description>`
- Return to base branch → `git checkout <configured-base-branch>`, then `git pull origin <configured-base-branch>` → `/zai-plan full <description>` (git mode only)
- Quick task → `/zai-plan fast <description>`
...
- Route it to `/zai-plan full <description>` without any git commands
...
**Read `.ai-factory/skill-context/zai-implement/SKILL.md`** — MANDATORY if the file exists.
...
- If `.ai-factory/skill-context/zai-implement/SKILL.md` does not exist and the resolved patches dir exists:
...
3. If file does not exist:
   "Plan file not found: <path>
    ...
    - /zai-implement @<resolved fast plan path>
    - /zai-implement @.ai-factory/plans/feature-user-auth.md"
   → STOP
4. If file is the resolved fix plan path:
   → invoke /zai-fix (ownership + cleanup workflow) and STOP
   ...
   → Resolve full-mode plan filename in this order:
     a. When `workflow.plan_id_format = sequential`, glob
        ...
        - >1 matches → use the **highest-numbered** match and emit
          `WARN [zai-implement] multiple sequential plans for <branch>: <list>; using <chosen>`.
...
2. If git mode is off or no branch-based plan is found above:
   - Check whether the configured plans dir contains exactly one `*.md` plan file created by `/zai-plan full` without a branch
...
4. No full-mode plan and no resolved fast plan → Check the resolved fix plan path
   → If exists: invoke /zai-fix (handles its own workflow with patches) and STOP
```

**Priority:**

1. `@<path>` argument - explicit user-selected plan file
2. Branch-named file (from `/zai-plan full`) - if it matches current branch
3. Single named full-plan file in `paths.plans` (from `/zai-plan full` without branch creation)
4. `paths.plan` (from `/zai-plan fast`) - fallback when no full plan exists
5. `paths.fix_plan` - redirect to `/zai-fix` (from `/zai-fix` plan mode)

**Note:** Plan discovery scans `paths.plans/` only. Plans archived to `paths.archive/plans/` by `/zai-archive` are excluded from discovery.
...
- Yes, commit now → run `make gpg-finalize COMMIT_MSG="..."` with the suggested message, then continue to next task
- No, continue to next task → proceed to the next task without committing
- Skip all commit checkpoints → for all subsequent checkpoints within this `/zai-implement` run, skip the prompt automatically and proceed directly to the next task (as if user selected "No, continue to next task" each time). This is in-context memory — resets on `/clear` or new session
...
2. If `HANDOFF_SKIP_REVIEW` is `1`: call `handoff_sync_status` with `{ taskId: <id>, newStatus: "done", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: false }`.
3. Otherwise: call `handoff_sync_status` with `{ taskId: <id>, newStatus: "review", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.
...
1. 🔍 /zai-verify — Verify nothing was missed (recommended)
2. 💾 /zai-commit — Commit the changes directly via make gpg-finalize
```
...
- If milestone mapping is ambiguous, emit `WARN [roadmap] ...` and suggest the owner command:
    - `/zai-roadmap check`
    - or `/zai-roadmap <short update request>`

**RULES.md (NOT allowed in this command):**

- Never edit the resolved `paths.rules_file` artifact from `/zai-implement`.
- If you discovered repeatable conventions/pitfalls during implementation, propose up to 3 candidate rules and ask the user to add them via `/zai-rules`.
- Do not invoke `/zai-rules` automatically (it is user-invoked).
...
Options:
1. Yes — output `/zai-rules ...` commands (recommended)
2. No — skip
```
...
Options:
1. Update existing docs (recommended) — invoke /zai-docs
2. Create a new feature doc page — invoke /zai-docs with feature-page context
3. Skip documentation
```

Handling:

- Option 1 → invoke `/zai-docs` to update README/docs based on completed work
- Option 2 → invoke `/zai-docs` with context to create `docs/<feature-slug>.md`, include sections (Summary, Usage/user-facing behavior, Configuration, API/CLI changes, Examples, Troubleshooting, See Also), and add a README docs-table link
- Option 3 → do not invoke `/zai-docs`; emit `WARN [docs] Documentation skipped by user`

If plan setting is `Docs: no` or setting is unset:

- Do **not** show a mandatory docs checkpoint prompt
- Do **not** invoke `/zai-docs` automatically
...
- **If the resolved fast plan path** (from `/zai-plan fast`):
...
- "No, I'll handle it manually" → show a reminder:
  ...
  To merge and clean up later:
    cd <main-repo-path>
    git merge <branch>
    /zai-plan --cleanup <branch>
  ```

#### Worktree Merge

1. **Ensure everything is committed** — check `git status`. If uncommitted changes exist, suggest `/zai-commit` first and wait.
...
Options:
1. Verify first — Run /zai-verify to check completeness (recommended)
2. Skip to commit — Go straight to commit (make gpg-finalize)
```

**Based on choice:**

- "Verify first" → invoke `/zai-verify` → after it completes, continue to context cleanup below
- "Skip to commit" → invoke `/zai-commit` → after it completes, continue to context cleanup below
...
```
/zai-implement @my-custom-plan.md
/zai-implement @.ai-factory/plans/feature-user-auth.md status
```
...
```
/zai-implement --without-plan add GET /healthz endpoint returning {"status":"ok"}
/zai-implement --without-plan rename LogLevel.VERBOSE to LogLevel.TRACE --docs=yes
```
...
- ✅ Follow `/zai-best-practices` guidelines (naming, structure, error handling)
