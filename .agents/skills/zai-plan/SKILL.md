...
- **On start:** Call `handoff_sync_status` with `{ taskId: <extracted-id>, newStatus: "planning", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.
- **On completion:** Call `handoff_push_plan` with `{ taskId: <extracted-id>, planContent: <full plan text> }`. Then call `handoff_sync_status` with `{ taskId: <extracted-id>, newStatus: "plan_ready", sourceTimestamp: "<current UTC time in ISO 8601 format>", direction: "zai_to_handoff", paused: true }`.
...
**Read `.ai-factory/skill-context/zai-plan/SKILL.md`** — MANDATORY if the file exists.
...
- This reduces ambiguity in `/zai-implement` milestone completion and `/zai-verify` roadmap gates

**OPTIONAL (recommended):** Read the resolved research path if it exists:

- Treat `## Active Summary (input for /zai-plan)` as an additional requirements source
...
Both are derived in a fixed order so the producer here and the branch-based consumers in `/zai-implement` / `/zai-improve` / `/zai-verify` / `/zai-rules-check` always agree on the filename.
...
- **`--parallel` scope (TL;DR — source-worktree scoped):**
  - **Where the prefix is computed:** the source worktree's `<configured plans dir>`
    (the repo where `/zai-plan` was invoked) — i.e. exactly here, in Step 1.2.c.
...
- **Archived plans are excluded from numbering.** Plans moved to `paths.archive/plans/` by `/zai-archive` are not in `<configured plans dir>` and therefore not counted. Archiving the highest-numbered plan frees that number for reuse.
...
4. Roadmap milestone linkage (only if the resolved roadmap artifact exists):
   ...
   b. Skip — no linkage (allowed; `/zai-verify --strict` should report WARN, not fail, for missing linkage alone)
...
Store all preferences — they will be used in the plan file and passed to `/zai-implement`.

Docs policy semantics:

- `Docs: yes` → `/zai-implement` MUST show a mandatory documentation checkpoint and route docs changes through `/zai-docs`
- `Docs: no` (or unset) → `/zai-implement` emits `WARN [docs]` and continues without a mandatory docs checkpoint
...
- Treat `--parallel` as disabled: do not create a worktree and do not auto-invoke `/zai-implement`.
...
**Full mode + parallel (`--parallel`):** Automatically invoke `/zai-implement` — the whole point of parallel is autonomous end-to-end execution in an isolated worktree. If `HANDOFF_BRANCH_PREPARED = 1`, treat `--parallel` as disabled and do not auto-invoke `/zai-implement`.

```
/zai-implement

CONTEXT FROM /zai-plan:
...
9. **Ownership boundary** – This command owns plan files only (the resolved fast plan path and files under `paths.plans`). Use owner commands (`/zai-roadmap`, `/zai-rules`, `/zai-explore`) for their artifacts.
...
- `/zai-implement` may offer deletion after completion
