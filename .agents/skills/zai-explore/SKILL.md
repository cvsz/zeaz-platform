...
All user-facing responses from `/zai-explore` MUST be written in `ui_language`.
...
**Read `.ai-factory/skill-context/zai-explore/SKILL.md`** — MANDATORY if the file exists.
...
The argument after `/zai-explore` can be:
...
- "This feels solid enough to plan. Want me to start `/zai-plan`?"
...
3. **Offer to capture when decisions are made**
   ...
   | New task/feature | Run `/zai-plan` | `paths.plan` or `paths.plans/<branch_stem-or-slug>.md` (or `paths.plans/<NNNN>_<branch_stem-or-slug>.md` under `plan_id_format: sequential`; `branch_stem` = current branch with `/` replaced by `-`) |
...
```
Save these exploration results to the resolved research path so we can /clear and /zai-plan can reuse them?
...
## Active Summary (input for /zai-plan)
...
```
User: /zai-explore add-auth-system
...
- **Flow into action**: "Ready to plan? Run `/zai-plan`"
...
**Next steps** (if ready):
- Create a plan: /zai-plan [fast|full] <description>
